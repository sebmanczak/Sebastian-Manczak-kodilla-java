...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        Board project = prepareTestData();
        //When
        List<TaskList> averageNumberOfDays = new ArrayList<>();
        averageNumberOfDays.add(new TaskList("In progress"));
        long sumOfDays = project.getTaskLists().stream()
                .filter(averageNumberOfDays::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(t -> t.getCreated())
        ...
        long numberOfTasks = project.getTaskLists().stream()
                ...
                .count();
        long avg = sumOfDays / numberOfTasks;
        //Then
        Assert.assertEquals(15.0, avg, 0.01);
    }
}
